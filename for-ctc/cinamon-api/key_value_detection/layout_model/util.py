...
def resize_to_prefered_width(image, preferred_w):
    h, w = image.shape[:2]
    # resize big image to preferred width
    if w > preferred_w:
        h = int(h * preferred_w / w)
        image = cv2.resize(image, (preferred_w, h), interpolation=cv2.INTER_AREA)
    return image

def resize_to_prefered_height(image, preferred_h):
    h, w = image.shape[:2]
    # resize big image to preferred heigth
    if h > preferred_h:
        w = int(w * preferred_h / h)
        image = cv2.resize(image, (w, preferred_h), interpolation=cv2.INTER_AREA)
    return image
